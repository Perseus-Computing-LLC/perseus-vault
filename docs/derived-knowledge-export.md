...
`perseus_vault_derived_export` creates a human-readable Markdown projection of durable knowledge while keeping SQLite as the only source of truth. The legacy `perseus_vault_derived_export` alias remains callable during the v2 compatibility window.
