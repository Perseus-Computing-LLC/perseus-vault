...
| M2 | **Before tagging 1.0 / GA** | (a) A full independent security audit; (b) signed releases with SLSA provenance; (c) close the remaining application-layer disclosure gap for metadata, embeddings, journal/state payloads, and blind-index leakage. | 🟡 In progress — protected FTS content indexing is addressed by #1180; (b) SLSA build provenance is wired into the release workflow (verify with `gh attestation verify … --repo …`); (a) and residual (c) remain open. |
