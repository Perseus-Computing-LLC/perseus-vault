...
`serve`, `write`, `prepare`, and `maintain` automatically use the standard key
path when it already exists (`~/.perseus-vault/secret.key`, or the legacy
`~/.mimir/secret.key` during migration). An explicit `--encryption-key` always
takes precedence. Client configuration generated by `connect`/`install-client`
also includes the detected key path, so an encrypted vault is not silently
opened for plaintext writes.

If an encrypted database is started without a key, Vault keeps the existing
mixed-operation behavior for compatibility but emits a warning that new bodies
will be plaintext. Treat that warning as an operator action: provide the key
before continuing.
