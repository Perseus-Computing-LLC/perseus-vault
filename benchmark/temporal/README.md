# Perseus Vault bi-temporal benchmark

A **reproducible, fully offline** check that Perseus Vault's bi-temporal facts behave
...
> if you can show it. When a fact is overwritten, Perseus Vault keeps the prior version
> in history (`entity_history`) and can answer *"what did we believe at time
> T?"* via `perseus_vault_as_of` — while normal recall stays live-only. This harness
...
| Check | Asserts |
|---|---|
| `as_of_mid_returns_v1` | `perseus_vault_as_of(t_mid)` returns the version live *between* the writes |
| `as_of_now_returns_v2` | `perseus_vault_as_of(now)` returns the current version |
| `as_of_before_not_found` | `perseus_vault_as_of(before-it-existed)` reports `found=false` |
| `recall_excludes_superseded_v1` | live `perseus_vault_recall` never returns the superseded content |
| `recall_includes_live_v2` | live `perseus_vault_recall` still returns the current content |
...
[`dataset.json`](./dataset.json) — `perseus_vault-temporal-mini`: a handful of everyday
...
`perseus_vault-temporal-mini`: **20/20 checks, 100% accuracy** — `as_of` returns the
...
|---|---|---|
| transaction-time | `perseus_vault_as_of` | "what did we **believe** at tx T" |
| valid/application-time | `perseus_vault_valid_at` | "what was **true** in the world at valid T, per current knowledge" |
| full bi-temporal | `perseus_vault_bitemporal` | "as of belief at tx_at, what was true at valid_at" |
