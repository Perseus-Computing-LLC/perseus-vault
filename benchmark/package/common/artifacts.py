...
_REPORT_KEYS = {
    ...
    "not_measured", "excluded", "public_evidence", "raw_inputs_captured",
    "network_calls", "result_signature_sha256", "claim_ids",
}
...
def _validate_safe_evidence(evidence: Any, path: str = "evidence") -> None:
    ...
    for key, value in evidence.items():
        ...
        if key in _SAFE_BOOLEAN_EVIDENCE_KEYS:
            ...
            if not isinstance(value, list) or not value or any(not isinstance(item, str) or not _SAFE_ID_RE.fullmatch(item) for item in value):
            ...
            if "private" in value.lower() or "query" in value.lower() or "token" in value.lower() or "credential" in value.lower():
                raise ValueError(f"{field_path} contains a private-looking label")
    ...
    for name, state in capabilities.items():
        ...
        for key in ("required_tools", "missing_tools"):
            if key in state and (
                ...
                or any(not isinstance(tool, str) or not _SAFE_ID_RE.fullmatch(tool) for tool in state[key])
            ):
        ...
        if state.get("reason") is not None and (not isinstance(state["reason"], str) or not _SAFE_ID_RE.fullmatch(state["reason"]) or any(token in state["reason"].lower() for token in ("private", "query", "token", "credential"))):
...
def validate_report(report: dict[str, Any]) -> None:
    ...
    if not isinstance(report["benchmark_id"], str) or not _SAFE_ID_RE.fullmatch(report["benchmark_id"]):
        raise ValueError("benchmark_id must be a bounded identifier")
    if not isinstance(report["suite_version"], str) or not _SAFE_ID_RE.fullmatch(report["suite_version"]):
        raise ValueError("suite_version must be a bounded identifier")
    for key in ("control_profile_sha256", "run_fingerprint_sha256", "result_signature_sha256"):
    ...
    if any(not isinstance(item, str) or not _SAFE_ID_RE.fullmatch(item) for item in report["not_measured"] + report["excluded"]):
    ...
    if "claim_ids" in report and (not isinstance(report["claim_ids"], list) or any(not isinstance(item, str) or not _SAFE_ID_RE.fullmatch(item) for item in report["claim_ids"])):
    ...
    for case in cases:
        ...
        case_id = case["id"]
        if not isinstance(case_id, str) or not _SAFE_ID_RE.fullmatch(case_id) or case_id in case_ids:
            raise ValueError("case IDs must be unique bounded identifiers")
        case_ids.add(case_id)
        if not isinstance(case["category"], str) or not _SAFE_ID_RE.fullmatch(case["category"]):
        ...
        if any(not isinstance(name, str) or not _SAFE_ID_RE.fullmatch(name) or not isinstance(value, bool) for name, value in checks.items()):
        ...
        if "failure_class" in case and (not isinstance(case["failure_class"], str) or not _SAFE_ID_RE.fullmatch(case["failure_class"])):
    ...
    for name, metric in metrics.items():
        if not isinstance(name, str) or not _SAFE_ID_RE.fullmatch(name) or not isinstance(metric, dict):
        ...
        if "reason" in metric and (not isinstance(metric["reason"], str) or not _SAFE_ID_RE.fullmatch(metric["reason"])):
    ...
    if report["status"] == "partial":
        if (
            not any(case["status"] in {"blocked", "unavailable", "not_measured"} for case in cases)
            and not any(metric.get("status") in {"partial", "unavailable", "not_measured"} for metric in metrics.values())
            and not any(state.get("status") in {"partial", "unavailable", "not_measured"} for state in report["capabilities"].values())
        ):
            raise ValueError("partial report contains no partial or unavailable evidence")
