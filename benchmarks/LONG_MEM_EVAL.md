...
> real `perseus_vault` binary over the public LongMemEval `_s` split and emits a signed
> `report.json`.
...
- The headline comparison mixed **different LLMs, judges, and splits** in one table
  (Perseus Vault on a 50-sample oracle split vs published systems on a 102-sample stratified
...
offline: **retrieval quality** (does Perseus Vault surface the gold evidence session?),
