...
const PROTECTED: [&str; 10] = [
    "error", "warn", "warning", "exception", "fatal", "panic", "denied", "refused", "timeout", "assertion", // traceback is covered below
];
...
pub fn digest(sha256: &str, bytes: &[u8], anchor: impl Fn(usize, usize) -> ArtifactAnchor) -> Result<EvidenceLogDigest, String> {
    let text = std::str::from_utf8(bytes).map_err(|_| "log digest requires a UTF-8 text artifact".to_string())?;
    ...
    for raw in text.split_inclusive('\n') {
        ...
        if protected(line) {
            ...
            groups.entry(key).and_modify(|v| { v.0 += 1; v.2 = a.clone(); }).or_insert((1, a.clone(), a));
        }
        start = end;
    }
    if text.is_empty() { total = 0; }
    let sections = groups.into_iter().map(|(template, (count, first, last))| DigestSection {
        kind: "collapsed_template".to_string(), template, count, first, last, omitted_occurrences: count.saturating_sub(2),
    }).collect::<Vec<_>>();
    let omitted_line_count = sections.iter().map(|s| s.omitted_occurrences).sum();
    Ok(EvidenceLogDigest { format: "perseus_vault_evidence_log_digest", source_sha256: sha256.to_string(), config_version: DIGEST_VERSION, input_line_count: total, omitted_line_count, protected_line_count: protected_lines.len(), sections, protected_lines, retrieval: "Use perseus_vault_artifact_excerpt with returned anchors for exact original bytes." })
}

#[cfg(test)]
mod tests {
    use super::*;
    fn anchor(s: usize, e: usize) -> ArtifactAnchor { ArtifactAnchor { sha256: "a".repeat(64), byte_start: s as i64, byte_end: e as i64, line_start: Some(1), line_end: Some(1) } }
    #[test]
    ...
    fn ci_deploy_service_and_repeat_fixtures_keep_every_protected_line() {
        ...
        for input in cases {
            ...
            for (line, _) in &digest.protected_lines {
                assert!(source.contains(line), "protected line must be verbatim: {line}");
            }
            let collapsed_count: usize = digest.sections.iter().map(|s| s.count).sum();
            assert_eq!(collapsed_count + digest.protected_line_count, source.lines().count());
        }
    }
}
