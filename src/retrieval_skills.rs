...
impl Default for RouterProfile {
    fn default() -> Self {
        Self { base: 1.0, recent: 0.0, negation: 0.0, question: 0.0, type_hint: 0.0, long_query: 0.0 }
    }
}
...
pub const RECENT_MARKERS: [&str; 9] =
    ["recent", "latest", "last", "today", "now", "yesterday", "this week", "newest", "current"];
pub const NEGATION_MARKERS: [&str; 7] =
    ["not", "never", "no longer", "without", "doesn't", "don't", "cannot"];
pub const QUESTION_MARKERS: [&str; 7] =
    ["who", "what", "when", "where", "why", "how", "which"];
pub const TYPE_HINT_MARKERS: [&str; 9] =
    ["policy", "decision", "fact", "convention", "lesson", "plan", "preference", "rule", "contact"];
...
pub fn query_features(query: &str) -> QueryFeatures {
    ...
    QueryFeatures {
        ...
        question: query.contains('?')
            || QUESTION_MARKERS.iter().any(|m| lower.starts_with(m)),
...
pub fn route_serving(
    ...
    let ranking: Vec<(String, f64)> = scored
        .into_iter()
...
mod tests {
    ...
    fn router_picks_affinity_and_ties_break_by_id() {
        let recency = skill("s-recency", RouterProfile { recent: 10.0, ..Default::default() }, "fts5");
    ...
    fn expansion_frontier_is_never_routed() {
        let mut expansion = skill("s-new", RouterProfile { base: 100.0, ..Default::default() }, "fts5");
    ...
    fn advance_gate_is_fail_closed() {
        let regress = AdvanceEvidence { eval_ref: "e1".into(), wins: 2, losses: 5, ties: 1, recall_delta: -0.03 };
        assert!(!regress.passes());
        let flat = AdvanceEvidence { eval_ref: "e2".into(), wins: 3, losses: 3, ties: 2, recall_delta: 0.0 };
        assert!(flat.passes());
        let wins = AdvanceEvidence { eval_ref: "e3".into(), wins: 9, losses: 1, ties: 0, recall_delta: 0.11 };
        assert!(wins.passes());
        let neg_delta = AdvanceEvidence { eval_ref: "e4".into(), wins: 9, losses: 1, ties: 0, recall_delta: -0.01 };
        assert!(!neg_delta.passes(), "negative metric delta refuses even with wins");
    }

    #[test]
    fn template_validation_is_fail_closed() {
        let ok = SkillTemplate { mode: "fts5".into(), limit: 10, ..SkillTemplate::default_() };
    ...
    fn skill_lifecycle_route_serve_audit() {
        ...
        write_fixture(&db, &skill_entity("mem-latest-a", "{\"content\": \"latest findings alpha\"}")).unwrap();
        write_fixture(&db, &skill_entity("mem-latest-b", "{\"content\": \"latest findings beta\"}")).unwrap();

        // Definitions enter the expansion frontier.
        let recency = simple_def("s-recency", RouterProfile { recent: 10.0, ..Default::default() });
        ...
        let regress = AdvanceEvidence { eval_ref: "e1".into(), wins: 1, losses: 4, ties: 0, recall_delta: -0.02 };
        let refused = db.skill_advance("s-recency", Some(&regress), "advance").unwrap();
        assert_eq!(refused["accepted"], false);
        // Still expansion → routing still fails.
        assert!(db.skill_route("latest results", false).is_err());

        // Non-regression evidence → accepted, serving, version bump.
        let good = AdvanceEvidence { eval_ref: "e2".into(), wins: 12, losses: 3, ties: 2, recall_delta: 0.31 };
        let adv = db.skill_advance("s-recency", Some(&good), "advance").unwrap();
        assert_eq!(adv["accepted"], true);
        assert_eq!(adv["serving_version"], 1);
        let adv2 = db.skill_advance("s-neutral", Some(&good), "advance").unwrap();
    ...
    fn skill_set_validation_is_fail_closed() {
        ...
        assert_eq!(db.skill_audit().unwrap()["skills"][0]["frontier"], "expansion");
    }
